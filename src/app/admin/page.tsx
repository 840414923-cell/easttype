      ...
      <div className="min-h-screen flex items-center justify-center bg-[#0f0d0a]">
        <div className="w-80 space-y-4">
          ...
          <input
            ...
            placeholder="输入密码"
            className="w-full px-4 py-2.5 rounded bg-[#2a2418] text-[#e8dcc8] border border-[#3a3428] outline-none focus:border-[#A63A3A]"
          />
          {loginError && <p className="text-red-400 text-sm text-center">{loginError}</p>}
          <button
            onClick={handleLogin}
            className="w-full py-2.5 rounded bg-[#A63A3A] text-[#0f0d0a] font-bold cursor-pointer hover:bg-[#d4a853]"
          >
            登录
          </button>
        </div>
    ...
    <div className="min-h-screen bg-[#0f0d0a] text-[#e8dcc8] p-6">
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <h1 className="text-2xl font-bold text-[#A63A3A]">EastType 后台管理</h1>
            ...
            <div className="flex bg-[#1e1a14] rounded-lg border border-[#2a2418] overflow-hidden">
              {([
                ...
                <button
                  ...
                  className={`px-5 py-2.5 text-sm cursor-pointer transition-colors ${tab === id ? "bg-[#A63A3A] text-[#0f0d0a] font-bold" : "text-[#7a6e5e] hover:text-[#e8dcc8]"}`}
                >
                  {label}
                </button>
              ))}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {Object.entries(stats).map(([ch, s]) => (
                <div key={ch} className="bg-[#1e1a14] border border-[#2a2418] rounded-lg p-4">
                  ...
                  <div className="text-2xl font-bold text-[#A63A3A] mt-1">已用 {s.used} / 总计 {s.total}</div>
        ...
        <div>
          ...
          <div className="flex gap-3 items-end">
            ...
            <button
              ...
              disabled={generating}
              className="px-6 py-2 rounded bg-[#A63A3A] text-[#0f0d0a] font-bold cursor-pointer hover:bg-[#d4a853] disabled:opacity-50"
            >
              {generating ? "生成中..." : "生成"}
            </button>
          </div>
          ...
          {generatedCodes.length > 0 && (
            <div className="mt-4 bg-[#1e1a14] border border-[#2a2418] rounded-lg p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <button onClick={copyCodes} className="text-xs text-[#A63A3A] cursor-pointer hover:underline">复制全部</button>
              </div>
              ...
              return Object.entries(groups).map(([date, group]) => (
                <div key={date} className="mb-4">
                  <div className="flex items-center justify-between mb-2 px-3">
                    <span className="text-sm font-bold text-[#A63A3A]">{date}</span>
              ...
              <div className="flex gap-2">
                {leads && leads.length > 0 && (
                  <button
                    ...
                    className="px-4 py-2 rounded bg-[#A63A3A] text-[#0f0d0a] text-sm font-bold cursor-pointer hover:bg-[#d4a853]"
                  >
                    导出 CSV
                  </button>
                )}
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                {doneCount > 0 && (
                  <button
                    ...
                    className="text-xs text-[#7a6e5e] cursor-pointer hover:text-[#A63A3A]"
                  >
                    重置进度
                  </button>
                )}
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 py-3 rounded-lg bg-[#A63A3A] text-[#0f0d0a] font-bold cursor-pointer hover:bg-[#d4a853] text-sm"
                >
              ...
              {genResult && (
                <div className="space-y-4 bg-[#1e1a14] border border-[#2a2418] rounded-lg p-5">
                  <div className="flex items-center justify-between">
                    <h3 className="text-sm font-bold text-[#A63A3A]">{genResult.title}</h3>
                  ...
                  <div className="text-xs text-[#7a6e5e] space-y-1">
                    <p><span className="text-[#A63A3A] font-bold">Cause:</span> {genResult.cause}</p>
                    <p><span className="text-[#A63A3A] font-bold">Symptoms:</span> {genResult.symptoms.join(" · ")}</p>
                    <p><span className="text-[#A63A3A] font-bold">Foods:</span> {genResult.foods.map((f) => `${f.en} (${f.zh})`).join(", ")}</p>
                  </div>

                  <div>
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-[10px] font-bold text-[#A63A3A] uppercase tracking-wider">Image2 画图提示词</span>
                      <button
                        onClick={() => handleCopyGen("prompt", genResult.imagePrompt)}
                        className="text-[10px] text-[#A63A3A] cursor-pointer hover:underline"
                      >
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-[10px] font-bold text-[#A63A3A] uppercase tracking-wider">Pinterest 文案</span>
                      <button
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-[10px] font-bold text-[#A63A3A] uppercase tracking-wider">Instagram 文案</span>
                      <button
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-[10px] font-bold text-[#A63A3A] uppercase tracking-wider">X (Twitter) 文案</span>
                      <button
                  ...
                  {genScript && (
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-[10px] font-bold text-[#A63A3A] uppercase tracking-wider">
