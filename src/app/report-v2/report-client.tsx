...
function MobileReading({ report, pro, lc, ui, ct, sex, typeId, isPro, upgradeToken }: {
  ...
  return (
    <div className="px-5 py-6 space-y-8 text-text leading-relaxed max-w-full overflow-hidden">
      ...
      {!isPro && (
        <div className="border-2 border-dashed border-accent rounded-xl p-5 text-center bg-[rgba(140,45,42,0.04)]">
          ...
          right={
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="text-sm text-accent font-semibold px-3 py-1.5 cursor-pointer bg-transparent border border-accent rounded-full hover:bg-[rgba(140,45,42,0.1)]"
              >
                {ui.downloadBtn}
              </button>
            </div>
...
function SectionHeader({ num, title }: { num: number; title: string }) {
  return (
    <div className="flex items-center gap-2 mb-3 pb-1.5 border-b border-[#e0d8cc]" style={{ breakAfter: "avoid" }}>
      <span className="text-[10px] font-bold text-white bg-[#A63A3A] w-5 h-5 rounded flex items-center justify-center flex-shrink-0">{num}</span>
  ...
  return (
    <div className="max-w-[210mm] mx-auto my-8 print:my-0 print:max-w-none">
      <div className="bg-white text-black shadow-2xl print:shadow-none rounded-lg overflow-hidden" id="report-a4">
        {/* Header */}
        <div className="border-b-2 border-[#A63A3A] px-12 py-6" style={{ breakAfter: "avoid", breakInside: "avoid" }}>
        ...
        <div className="px-12 py-6">
          ...
          {!isPro && (
            <>
              ...
              <div className="mb-6" style={{ breakInside: "avoid" }}>
                ...
                <div className="text-[10px] font-bold text-[#A63A3A] uppercase tracking-wider mb-2">{a4ui.recommended}</div>
          ...
          {isPro && pro && (
            <>
              ...
              <div className="mb-6" style={{ breakInside: "avoid" }}><SectionHeader num={6} title={a4ui.proCh4Acupoints} /><div className="space-y-3">{pro.ch4Acupoints.map((point, i) => (
                <div key={i} className="border border-[#e0d8cc] rounded px-4 py-2.5 border-l-[3px] border-l-[#A63A3A]"><div className="text-[13px] text-[#444] leading-[1.7]" dangerouslySetInnerHTML={{ __html: renderMd(t(point)) }} /></div>
              ))}</div></div>
              <div className="mb-6" style={{ breakInside: "avoid" }}><SectionHeader num={7} title={a4ui.proCh6Recipes} /><div className="space-y-4">{pro.ch6Recipes.map((recipe, i) => (
                <div key={i} style={{ breakInside: "avoid" }}><div className="text-[12px] font-bold text-[#A63A3A] uppercase tracking-wider mb-1.5">{a4ui.proRecipeN} {i + 1}</div><A4RecipeCard recipe={recipe} lc={lc} ui={a4ui} /></div>
              ))}</div></div>
          ...
          {/* Upgrade for basic */}
          {!isPro && (
            <div className="print:hidden border-2 border-dashed border-[#A63A3A] rounded-lg px-6 py-4 mt-6 text-center bg-[#fdf9f3]">
              <div className="text-[10px] font-bold text-[#A63A3A] uppercase tracking-wider mb-1">{a4ui.proUpgradeTitle}</div>
              ...
              <CreemCheckout
                ...
                metadata={{ type: typeId, sex, plan: "pro" }}
              >
                <div className="inline-flex items-center px-5 py-1.5 rounded text-[13px] font-semibold cursor-pointer bg-[#A63A3A] text-white">
                  {a4ui.proUpgradeCta}
                </div>
              </CreemCheckout>
            </div>
        ...
        <div className="bg-[#f8f6f2] px-12 py-1.5 text-center text-[11px] text-[#A63A3A] font-semibold tracking-wider border-t border-[#e0d8cc]">www.myeasterntype.com</div>
      </div>
    </div>
  )
}
